...
namespace Zink.Services
{
    ...
    public static class ScreenShareReportService
    {
        ...
        public static string DocumentsScreenShareDirectoryPath
        {
            get
            {
                var documentsPath = Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments);
                return string.IsNullOrWhiteSpace(documentsPath)
                    ? Path.Combine(DiagnosticLogService.LogDirectoryPath, "Screen Share")
                    : Path.Combine(documentsPath, "Zink", "Screen Share");
            }
        }

        public static async Task<string> SaveLatestStateAsync(ScreenShareReportContext context)
        {
            DiagnosticLogService.InitializeFromSettings();
            DiagnosticLogService.Flush();

            var reportDirectory = GetReportDirectory(context);
            Directory.CreateDirectory(reportDirectory);

            var deviceName = DiagnosticLogService.DeviceName;
            var statePath = Path.Combine(reportDirectory, $"zink-screen-share-state-{deviceName}-latest.txt");
            await File.WriteAllTextAsync(statePath, await BuildReportAsync(context, latestState: true), Utf8NoBom);
            CopyCurrentDiagnosticLogToReportDirectory(reportDirectory);
            return statePath;
        }
        ...
        public static async Task<string> CreateBundleAsync(ScreenShareReportContext context)
        {
            ...
            var reportPrefix = string.Equals(context.SessionType, "Call", StringComparison.OrdinalIgnoreCase)
                ? "zink-call"
                : "zink-screen-share";
            var reportPath = Path.Combine(reportDirectory, $"{reportPrefix}-{deviceName}-{stamp}.txt");
            var bundlePath = Path.Combine(reportDirectory, $"{reportPrefix}-{deviceName}-{stamp}.zip");

            await File.WriteAllTextAsync(reportPath, await BuildReportAsync(context, latestState: false), Utf8NoBom);
            CopyCurrentDiagnosticLogToReportDirectory(reportDirectory);
        ...
        private static async Task<string> BuildReportAsync(ScreenShareReportContext context, bool latestState)
        {
            var userInfo = await TokenStore.Instance.GetUserInfoAsync();

            var builder = new StringBuilder();
            builder.AppendLine(latestState
                ? "Zink screen-share latest state"
                : $"Zink {context.SessionType.ToLowerInvariant()} feedback report");
            ...
            if (IsScreenShareReport(context))
                builder.AppendLine("  Screen-share documents folder: " + DocumentsScreenShareDirectoryPath);
        ...
        private static string GetReportDirectory(ScreenShareReportContext context)
        {
            return IsScreenShareReport(context)
                ? DocumentsScreenShareDirectoryPath
                : DiagnosticLogService.LogDirectoryPath;
        }

        private static bool IsScreenShareReport(ScreenShareReportContext context)
        {
            return string.Equals(context.SessionType, "Screen share", StringComparison.OrdinalIgnoreCase);
        }

        private static void CopyCurrentDiagnosticLogToReportDirectory(string reportDirectory)
        {
            try
            {
                var sourcePath = DiagnosticLogService.CurrentLogPath;
                if (!File.Exists(sourcePath))
                    return;

                Directory.CreateDirectory(reportDirectory);
                var targetPath = Path.Combine(reportDirectory, Path.GetFileName(sourcePath));
